...
void main() {
  test('sets Access-Control-Allow-Origin for simple requests', () async {
    final ctx = TestContext.get('/', headers: {'origin': 'https://example.com'});
  ...
  test('echoes origin when credentials are allowed', () async {
    final ctx = TestContext.get('/', headers: {'origin': 'https://example.com'});
    await buildMiddlewareChain([Cors(const CorsOptions(allowCredentials: true))], (_) async {})(ctx);

    expect(ctx.response.headers.value('access-control-allow-origin'), 'https://example.com');
    expect(ctx.response.headers.value('access-control-allow-credentials'), 'true');
    expect(ctx.response.headers.value('vary'), 'Origin');
  });

  test('handles preflight requests', () async {
    final ctx = TestContext.create('OPTIONS', '/', headers: {
    ...
    var called = false;
    await buildMiddlewareChain([Cors(const CorsOptions(maxAge: Duration(hours: 1)))], (_) async {
      called = true;
    })(ctx);

    expect(called, isFalse);
    expect(ctx.response.statusCode, HttpStatus.noContent);
    expect(ctx.response.headers.value('access-control-allow-methods'), contains('POST'));
    expect(ctx.response.headers.value('access-control-allow-headers'), 'X-Test');
  ...
  group('Origin validation', () {
    test('allows specific origin when in allowed list', () async {
      ...
      await buildMiddlewareChain([
        Cors(const CorsOptions(origins: ['https://example.com', 'https://test.com'])),
      ], (_) async {})(ctx);
      expect(ctx.response.headers.value('access-control-allow-origin'), 'https://example.com');
    });
    ...
    test('allows any origin when wildcard in list', () async {
      final ctx = TestContext.get('/', headers: {'origin': 'https://any-domain.com'});
      await buildMiddlewareChain([Cors(const CorsOptions(origins: ['*']))], (_) async {})(ctx);
      expect(ctx.response.headers.value('access-control-allow-origin'), 'https://any-domain.com');
    });

    test('uses callback for dynamic origin validation', () async {
      final ctx = TestContext.get('/', headers: {'origin': 'https://sub.example.com'});
      await buildMiddlewareChain([
        Cors(CorsOptions(originCallback: (o) => o?.endsWith('.example.com') ?? false)),
      ], (_) async {})(ctx);
      expect(ctx.response.headers.value('access-control-allow-origin'), 'https://sub.example.com');
    });
  ...
  group('Headers configuration', () {
    test('sets expose headers when configured', () async {
      final ctx = TestContext.get('/', headers: {'origin': 'https://example.com'});
      await buildMiddlewareChain([
        Cors(const CorsOptions(exposeHeaders: ['X-Total-Count', 'X-Page-Number'])),
      ], (_) async {})(ctx);
      expect(ctx.response.headers.value('access-control-expose-headers'), 'X-Total-Count, X-Page-Number');
    });
    ...
    test('allows custom methods in preflight', () async {
      ...
      await buildMiddlewareChain([
        Cors(const CorsOptions(allowMethods: ['GET', 'POST', 'DELETE', 'PATCH'])),
      ], (_) async {})(ctx);
      expect(ctx.response.headers.value('access-control-allow-methods'), 'GET, POST, DELETE, PATCH');
    });
    ...
    test('allows custom headers in preflight', () async {
      ...
      await buildMiddlewareChain([
        Cors(const CorsOptions(allowHeaders: ['Content-Type', 'Authorization', 'X-Custom'])),
      ], (_) async {})(ctx);
      expect(ctx.response.headers.value('access-control-allow-headers'), 'Content-Type, Authorization, X-Custom');
    });
    ...
    test('mirrors request headers when no allow headers configured', () async {
      ...
      await buildMiddlewareChain([Cors()], (_) async {})(ctx);
      expect(ctx.response.headers.value('access-control-allow-headers'), 'X-Custom-Header, X-Another');
    });
  });
  ...
  group('Edge cases', () {
    ...
    test('max age duration converts to seconds correctly', () async {
      ...
      await buildMiddlewareChain([Cors(const CorsOptions(maxAge: Duration(days: 1)))], (_) async {})(ctx);
